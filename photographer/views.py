...
def login(request):
    ...
    try:
        try:
            find_user = photographer.objects.get(email=get_email)
...
def register(request):
    ...
    try:
        if photographer.objects.filter(graph_name=get_username).count() > 0:
            response = {
                "error_code": 10000,
                "message": "user already exists",
            }
        elif photographer.objects.filter(graph_name=get_username).count() == 0:
